  ...
  <div class="divBox">
    <el-card class="box-card">
      ...
      <el-table :data="tableData.data" v-loading="loading" size="mini"
        ...
        <el-table-column align="center" label="轮播图" prop="imageUrl" min-width="50">
          <template slot-scope="scope">
            <el-image :src="baseURL+scope.row.imageUrl"
        ...
        <el-table-column align="center" label="链接地址" min-width="100">
          <template slot-scope="scope">
            <span v-if="scope.row.linkAddress">{{ scope.row.linkAddress }}</span>
            <span v-else>-</span>
          </template>
        </el-table-column>
    ...
    <el-dialog
      ...
      :title="editDialogConfig.isCreate === 0? '添加轮播图':'编辑轮播图'"
      destroy-on-close
...
export default {
  ...
  methods: {
    checkPermi,
    onChangeStatus(row) {
      saveRow({id: row.id, status: row.status}).then(() => {
