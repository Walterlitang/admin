  ...
  <div class="divBox">
    <el-card class="box-card">
        ...
        <div class="container">
          <el-form inline size="small" @submit.native.prevent>
            ...
            <el-form-item class="el-form-item-align-right">
              <el-button icon="el-icon-circle-plus" size="mini" type="primary" @click="openEdit(null,null)" v-has-permi="['admin:friendshipLink:classification:save']">添加
              </el-button>
            </el-form-item>
          </el-form>
        </div>
      ...
      >
        ...
        <el-table-column label="URL路径" prop="urlPath" min-width="100"/>
        <el-table-column label="操作" min-width="180" fixed="right" align="center">
          <template slot-scope="scope">
            <el-button
                ...
                size="mini"
                type="text"
                @click="openEdit(null,scope.row.id)"
                v-has-permi="['admin:friendshipLink:classification:save']">
              添加子级
            </el-button>
            <el-button
                icon="el-icon-edit"
                size="mini"
                type="text"
                @click="openEdit(scope.row.id,scope.row.id)"
                v-has-permi="['admin:friendshipLink:classification:save']">
              编辑
            </el-button>
<!--            <el-button-->
...
<!--                v-has-permi="['admin:friendshipLink:classification:delete']"-->
...
import {getTree,deleteRow,updateStatus} from "@/api/friendshipLink";
...
export default {
  ...
  methods: {
    initializeExpandedRows() {
      this.expandedRowKeys = this.treeList.map(item => item.id);
    },
