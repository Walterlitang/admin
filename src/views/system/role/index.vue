    ...
    <el-card class="box-card">
      <el-form inline size="small" @submit.native.prevent>
        <el-form-item label="关键词：">
          <el-input v-model="listPram.keyword" placeholder="请输入角色昵称" clearable class="selWidth"/>
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item class="el-form-item-align-right">
          <el-button icon="el-icon-circle-plus" size="mini" type="primary" @click="openEdit()" v-has-permi="['admin:system:role:save']">添加角色</el-button>
        </el-form-item>
      </el-form>
      <el-table :data="listData.list" size="mini" :header-cell-style=" {fontWeight:'bold',background:'#f8f8f9',color: '#515a6e',height:'40px'}" border>
        ...
        <el-table-column label="状态" prop="status">
          <template slot-scope="scope">
            <el-switch
                :disabled="!checkPermi(['admin:system:role:status'])"
        ...
        <el-table-column label="操作" min-width="150" fixed="right" align="center">
          <template slot-scope="scope">
            <el-button icon="el-icon-edit" size="mini" type="primary" @click="openEdit(scope.row.id)"  v-has-permi="['admin:system:role:save']">编辑</el-button>
            <el-button icon="el-icon-delete" size="mini" type="danger"
                :disabled="isIdBetween1And5(scope.row.id)"
                @click="handlerDelete(scope.row.id)"
                v-has-permi="['admin:system:role:delete']"
            >
              删除
            </el-button>
          </template>
...
import {deleteRow, list, updateStatus} from "@/api/role";
