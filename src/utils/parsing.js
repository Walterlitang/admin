 ...
 export function formatRoutes(routerArr){
    ...
    //console.log(tmp);return false;
		obj = {
			...
			//path:'/' + tmp.pid + '/',
			perms:tmp.perms,
			child:(tmp.childList && tmp.childList.length) ? tmp.childList.map(item=>{
				return {
					id:item.id,
					pid:item.pid,
					name:item.name,
					...
					perms:item.perms,
					extra:item.icon,
					child:(item.childList && item.childList.length) ? item.childList.map(item1=>{
						return {
							id:item1.id,
							pid:item1.pid,
							name:item1.name,
							...
							perms:item1.perms,
							extra:item1.icon,
							child:(item1.childList && item1.childList.length) ? item1.childList.map(item2=>{
								return {
									id:item2.id,
									pid:item2.pid,
									name:item2.name,
