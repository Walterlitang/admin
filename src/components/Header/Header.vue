...
<div class="header content">
  ...
  <div class="icon-container">
    <div v-for="(icon, index) in icons" :key="icon.fid" class="icon-item" @click="handleIconClick(icon)">
      <div class="icon-wrapper">
        <img :src="imgUrl + icon.icon" alt="icon"/>
      </div>
      <div class="icon-text">{{ icon.title }}</div>
    </div>
  </div>


  <template v-if="this.$route.path!=='/mailbox'">
    <div class="banner">
      <el-carousel indicator-position="none" class="banner"  height="120px">
        <el-carousel-item v-for="item in banner" :key="item.id" >
          <img :src="imgUrl+item.imageUrl" alt=""  class="img" @click="goExternal(item.linkAddress)"/>
        </el-carousel-item>
      </el-carousel>
    </div>
    <div class="banner" style="margin-top: 10px">
      <el-carousel indicator-position="none" class="banner"  height="120px">
        <el-carousel-item v-for="item in banner1" :key="item.id">
          <img :src="imgUrl+item.imageUrl" alt="" class="img" @click="goExternal(item.linkAddress)"/>
        </el-carousel-item>
      </el-carousel>
    </div>
...
export default {
  ...
  methods: {
    ...
    handleIconClick(icon) {
      if (icon.linkAddress) {
        window.open(icon.linkAddress, '_blank'); // 在新标签页中打开链接
      }
    }
  },
  mounted(){
